package Model.Decorator.ConcreteDecorators;
...
public class Gancho extends Golpe {

    public Gancho(Ataque ataque) {
        super(ataque);
        setDanoGolpe(5);
    }

    public Gancho(Personagem personagem){
        setDanoGolpe(5);
    ...
    public void atacar(Personagem atacado) {
        ...
        if(getAtaque() == null || (getAtaque() != null && getAtacante()!= null && getAtacante().getAtaqueInicial() != getAtaque())){
            setAtaque(getAtacante().getAtaqueInicial());//Ataque Inicial (FRACO, NORMAL E FORTE)
        }
